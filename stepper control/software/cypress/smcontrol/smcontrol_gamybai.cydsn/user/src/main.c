...
static const uint8 CYCODE EE_Data = 0x15;



/* Function prototypes */
void PwmTimerIsrConfiguration( void );
void DI0_IsrConfiguration( void );
...
/**/
typedef enum { STATE_A = 0, STATE_B, STATE_C, STATE_D } StateType_TypeDef;

void StateFunc_A(void);
void StateFunc_B(void);
void StateFunc_C(void);
void StateFunc_D(void);

void (*state_table[])(void) = { StateFunc_A, StateFunc_B, StateFunc_C, StateFunc_D };
StateType_TypeDef CurrState = STATE_A;
...
/*  */
void DI0_IsrConfiguration()
{
    DI0_ISR_StartEx(IN0_IsrHandler);
    DI0_ISR_Enable();
    DI0_ISR_ClearPending();   
}
